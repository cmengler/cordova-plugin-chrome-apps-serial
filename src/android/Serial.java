...
public class Serial extends CordovaPlugin {
	// logging tag
    private static final String TAG = "ChromeSerial";
    ...
    public boolean execute(String action, CordovaArgs args, final CallbackContext callbackContext) throws JSONException {
        ...
        } else if ("openSerial".equals(action)) {
            JSONObject opts = arg_object.has("opts")? arg_object.getJSONObject("opts") : new JSONObject();
            openSerial(args, callbackContext);
        } else if ("writeSerial".equals(action)) {
            String data = arg_object.getString("data");
			writeSerial(data, callbackContext);
        } else if ("writeSerialHex".equals(action)) {
            String data = arg_object.getString("data");
			writeSerialHex(data, callbackContext);
        } else if ("readSerial".equals(action)) {
            readSerial(callbackContext);
        } else if ("closeSerial".equals(action)) {
            closeSerial(callbackContext);
        } else if ("registerReadCallback".equals(action)) {
